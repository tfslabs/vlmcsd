...
static BOOL setHwIdFromIniFileLine(const char **s, const uint32_t index, const uint8_t overwrite)
{
	...
	if (**s == '/')
	{
		if (!overwrite && KmsResponseParameters[index].HwId)
			return TRUE;
...
static BOOL setEpidFromIniFileLine(const char **s, const uint32_t index, const char *ePidSource, const uint8_t overwrite)
{
	...
	if (i < 1 || i >= PID_BUFFER_SIZE)
		return FALSE;
	if (!overwrite && KmsResponseParameters[index].Epid)
		return TRUE;
...
static BOOL handleIniFileEpidParameter(const char *s, uint8_t allowIniFileDirectives, const char *ePidSource)
{
	...
	if (i >= 0)
	{
		if (!getIniFileArgument(&s))
			return FALSE;
		if (!setEpidFromIniFileLine(&s, i, ePidSource, !allowIniFileDirectives))
			return FALSE;
		if (!setHwIdFromIniFileLine(&s, i, !allowIniFileDirectives))
			return FALSE;
		return TRUE;
	}
...
static void ignoreIniFileParameter(uint_fast8_t iniFileParameterId)
{
	...
	{
		if (IniFileParameterList[i].Id != iniFileParameterId)
			continue;
...
static BOOL readIniFile(const uint_fast8_t pass)
{
	...
	for (lineNumber = 1; (s = fgets(line, sizeof(line), f)); lineNumber++)
	{
		...
		{
			if (handleIniFileParameter(s))
				continue;
			lineParseError = TRUE;
		}
		else if (pass == INI_FILE_PASS_2)
		{
			if (handleIniFileEpidParameter(s, TRUE, fn_ini))
				continue;
			lineParseError = TRUE;
		}
#if !defined(NO_SOCKETS) && !defined(SIMPLE_SOCKETS) && !defined(USE_MSRPC)
...
__noreturn static void HangupHandler(const int signal_unused)
{
	...
	for (i = 0; i < argc_in; i++)
	{
		if (!strcmp(argv_in[i], "-Z"))
			daemonize_protection = FALSE;
		argv_out[i] = argv_in[i];
	}

	argv_out[argc_in] = argv_out[argc_in + 1] = NULL;
	if (daemonize_protection)
		argv_out[argc_in] = (char *)"-Z";
...
static void parseGeneralArguments()
{
	...
	for (opterr = 0; (o = getopt(global_argc, (char *const *)global_argv, (const char *)optstring)) > 0;)
		...
		case 'T':
			if (!getArgumentBool(&LogDateAndTime, optarg))
				usage();
...
#if HAVE_FREEBIND
		case 'F':
			if (!getArgumentBool(&freebind, optarg))
				usage();
...
#endif // USE_MSRPC
			if (!IsNTService)
				installService = 1; // Install
			break;

		case 'S':
			if (!IsNTService)
				installService = 2; // Remove
			break;
#endif // _NTSERVICE
		...
		case 'c':
			if (!getArgumentBool(&CheckClientTime, optarg))
				usage();
...
#ifndef NO_CLIENT_LIST
		case 'E':
			if (!getArgumentBool(&StartEmpty, optarg))
				usage();
			ignoreIniFileParameter(INI_PARAM_START_EMPTY);
			break;

		case 'M':
			if (!getArgumentBool(&MaintainClients, optarg))
				usage();
...
#ifndef SIMPLE_RPC
		case 'N':
			if (!getArgumentBool(&UseServerRpcNDR64, optarg))
				usage();
			IsNDR64Defined = TRUE;
		...
		case 'B':
			if (!getArgumentBool(&UseServerRpcBTFN, optarg))
				usage();
	...
	if ((ServiceUser || *ServicePassword) && installService != 1)
		usage();
#endif // _NTSERVICE
}
...
void cleanup()
{
	if (!InetdMode)
	{
		...
		if (shmid >= 0)
		{
			if (MaxTaskSemaphore != (sem_t *)-1)
				shmdt(MaxTaskSemaphore);
...
int setupListeningSockets()
{
	...
	int numPrivateIPs = 0;
	if (PublicIPProtectionLevel & 1)
		getPrivateIPAddresses(&numPrivateIPs, &privateIPList);
	...
	{
#if HAVE_GETIFADDR
		if (!(PublicIPProtectionLevel & 1) && haveIPv6Stack)
			addListeningSocket("::");
		if (!(PublicIPProtectionLevel & 1) && haveIPv4Stack)
...
// Initialize the Critical Section for proper logging
...
#ifndef NO_TAP
	if (tapArgument && !InetdMode)
		startTap(tapArgument);
#endif // NO_TAP
	...
	if (!InetdMode)
	{
		...
		if ((error = listenOnAllAddresses()))
			return error;
#else  // !SIMPLE_SOCKETS
		if ((error = setupListeningSockets()))
			return error;
#endif // !SIMPLE_SOCKETS
	}
#endif // !defined(NO_SOCKETS) && !defined(USE_MSRPC)
